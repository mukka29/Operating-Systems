...
static int spawn_user(){
  ...
  if(pid == -1){
    ...
    if(q_enq(&fb_ready[0], id) < 0){
      printf("[%ld:%i] Error: Enqueue of process with PID %d failed\n", data->timer.tv_sec, data->timer.tv_usec, usr->pid);
    }else{
      printf("[%ld:%i] OSS: Generating process with PID %u and putting it in queue 0\n", data->timer.tv_sec, data->timer.tv_usec, usr->id);
    }
  }
...
static int schedule_blocked(){
  ...
  if(q_enq(&fb_ready[0], id) < 0){
    fprintf(stderr, "[%ld:%i] OSS: Failed to enqueue process with PID %d\n", data->timer.tv_sec, data->timer.tv_usec, usr->id);
    return -1;
  }else{
    printf("[%ld:%i] OSS: Unblocked process with PID %d to queue 0\n", data->timer.tv_sec, data->timer.tv_usec, usr->id);
    return 0;
  }
}
...
static void idle_mode_off(){
  ...
  printf("[%ld:%i] OSS: IDLE OFF (lasted %ld:%i)\n", data->timer.tv_sec, data->timer.tv_usec, tv.tv_sec, tv.tv_usec);
...
static void time_jump(){
  ...
  if(q_len(&blocked) == 0){ //no blocked processes
    //fork time jump
    printf("[%ld:%i] OSS: Jump to fork timer at %ld:%i\n", data->timer.tv_sec, data->timer.tv_usec, timer_fork.tv_sec, timer_fork.tv_usec);
    ...
    struct user_pcb * usr = &data->users[id];
    printf("[%ld:%i] OSS: No process ready. Setting time to first unblock at %ld:%i\n",
      data->timer.tv_sec, data->timer.tv_usec, usr->t[T_BLOCKED].tv_sec, usr->t[T_BLOCKED].tv_usec);
...
static int schedule_dispatch(const int q){
  ...
  printf("[%ld:%i] OSS: Dispatching process with PID %u from queue %i\n", data->timer.tv_sec, data->timer.tv_usec, usr->id, q);
  ...
  switch(usr->state){
    case ST_TERM:
      ...
      printf("[%ld:%i] OSS: Process with PID %u terminated\n", data->timer.tv_sec, data->timer.tv_usec, usr->id);

      timerinc(&T[TS_TURN],    &usr->t[T_SYS]);
      timersub(&usr->t[T_SYS], &usr->t[T_CPU], &tv);
      timerinc(&T[TS_WAIT], &tv);

      printf("[%ld:%i] OSS: Process with PID %u terminated, removed from queue %d\n", data->timer.tv_sec, data->timer.tv_usec, usr->id, q);
    ...
    case ST_BLOCKED:
      printf("[%ld:%i] OSS: Process with PID %u has blocked on IO to %ld:%i\n", data->timer.tv_sec, data->timer.tv_usec, usr->id, usr->t[T_BURST].tv_sec, usr->t[T_BURST].tv_usec);

      timerinc(&usr->t[T_BLOCKED], &usr->t[T_BURST]);
      timerinc(&usr->t[T_BLOCKED], &data->timer);

      printf("[%ld:%i] OSS: Putting process with PID %u into blocked queue\n", data->timer.tv_sec, data->timer.tv_usec, usr->id);
      q_enq(&blocked, id);
      break;
    ...
    case ST_READY:
      ...
      if(mbuf.exec_time.tv_usec == fb_quant[q]){  //if not preepted
        ...
        nextq = q;
        printf("[%ld:%i] OSS: not using its entire time quantum\n", data->timer.tv_sec, data->timer.tv_usec);
      }
      usr->t[T_READY] = data->timer;

      printf("[%ld:%i] OSS: Receiving that process with PID %u ran for %lu nanoseconds\n", data->timer.tv_sec, data->timer.tv_usec, usr->id, usr->t[T_BURST].tv_usec);
      timerinc(&usr->t[T_CPU], &usr->t[T_BURST]);
      timerinc(&data->timer,   &usr->t[T_BURST]);

      printf("[%ld:%i] OSS: Process with PID %u moved to queue %d\n", data->timer.tv_sec, data->timer.tv_usec, usr->id, nextq);
  ...
  printf("[%ld:%i] OSS: total time this dispatching was %lu nanoseconds\n", data->timer.tv_sec, data->timer.tv_usec, tv.tv_usec);
